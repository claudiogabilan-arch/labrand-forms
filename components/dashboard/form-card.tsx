...
export function FormCard({ form, responseCount }: FormCardProps) {
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg hover:shadow-amber-500/10 transition-all duration-300 hover:-translate-y-0.5 bg-white/80 backdrop-blur-sm border-slate-200/60">
      {/* Color accent bar */}
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <Link 
            ...
            className="text-lg font-semibold text-slate-900 hover:text-amber-700 truncate block transition-colors"
          >
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 flex items-center gap-2">
        <Link href={`/forms/${form.id}/edit`} className="flex-1">
          <Button variant="outline" size="sm" className="w-full hover:bg-amber-50 hover:text-amber-800 hover:border-amber-200 transition-colors">
            <Pencil className="w-3 h-3 mr-2" />
            Editar
          </Button>
        </Link>
        <Link href={`/forms/${form.id}/responses`} className="flex-1">
          <Button variant="outline" size="sm" className="w-full hover:bg-stone-50 hover:text-stone-700 hover:border-stone-200 transition-colors">
